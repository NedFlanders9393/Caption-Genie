...
- `eas submit` has no `--no-wait`: it prints "Scheduled iOS submission" + a submissions URL, then BLOCKS on "Waiting for submission to complete." The 120s bash timeout killing the CLI is HARMLESS — the submission already runs server-side on EAS once scheduled. Do NOT re-run submit (would create a duplicate). Track it via the printed `expo.dev/.../submissions/<id>` URL. `eas submit:list` does NOT exist in this CLI version; `eas build:view <id>` still works for the build itself.
